...
enum system_state_t STATE;
extern char topic_msg[100];
extern char topic_actionack[100];
	...
	while (true) {
		if (xQueueReceive(button_events, &ev, 1000/portTICK_PERIOD_MS)) {
			if ((ev.pin == BUTTON) && (ev.event == BUTTON_DOWN)) {
				...
				if (STATE == NORMAL) {
					char data[100] = {'\0'};
					sprintf(data, "{\"power_%d\":\"%s\",\"endpoint\":1}", 1, (value == 1) ? "on":"off");
					esp_mqtt_client_publish(client, topic_msg, data, strlen(data), 0, 0);
					esp_mqtt_client_publish(client, topic_actionack, data, strlen(data), 0, 0);
